...
/** 设备标记数据（与 embed/types.ts Device 接口兼容） */
...
const emit = defineEmits<{
  ...
  'scope-change': [scope: MapScope];
}>();

// 设备标记 CSS2DObject 追踪
let deviceMarkerGroup: THREE.Group | undefined;
const deviceMarkerElements = new Map<string, HTMLDivElement>();
...
function applyCameraView(view: CameraViewPreset) {
  ...
  if (controls) {
    ...
    controls.update();
    // 视野归位到本层级预设后，按自然视距刷新缩放上下限，
    // 保证「缩到最远」对应的是当前层级完整区域，而非固定的全局最大值。
    applyZoomLimitsForCurrentScope();
  } else {
...
/**
 * 返回当前缩放状态，供外层 CockpitMap 判断「是否已缩放到最远（atMax）」，
 * 从而仅在完全缩小后再用滚轮向上一级（避免一滚就跳出当前层级）。
 * - minDistance = 完全放大（最近）
 * - maxDistance = 完全缩小（最远）
 * atMax 阈值用 maxDistance-50，确保一旦接近最远即触发返回。
 */
function getZoomState() {
  if (!camera || !controls) {
    return { distance: 0, min: 0, max: 0, atMin: false, atMax: false };
  }
  ...
  return {
    ...
    atMin: distance <= controls.minDistance + 40,
    atMax: distance >= controls.maxDistance - 50,
  };
}

/**
 * 手动缩放：由外层 CockpitMap 在拦截滚轮后调用。
 * 直接修改 camera.position 沿 target 方向，即时生效（无 damping 延迟），
 * 确保 getZoomState() 反映的是真实、最新的相机距离。
 */
function zoomBy(deltaY: number) {
  ...
  const factor = Math.exp(deltaY * 0.001 * (controls.zoomSpeed || 1));
...
/**
 * 根据当前层级「正好铺满视野」的自然视距动态设置缩放上下限。
 * 每一级的地图几何体都重新投影、居中并铺满同一片可视区域，
 * 因此各层级的自然视距基本一致。把最远距设为自然视距的 1.12 倍，
 * 这样「缩到最远（看到完整区域 + 少量留白）」即触发 atMax，
 * 外层滚轮再向下滚即可返回上一级。
 */
function applyZoomLimitsForCurrentScope() {
  if (!camera || !controls) return;
  const natural = camera.position.distanceTo(controls.target);
  if (!Number.isFinite(natural) || natural <= 0) return;
  controls.minDistance = Math.max(200, natural * 0.4);
  controls.maxDistance = natural * 1.12;
}
...
/**
 * 创建设备标记 CSS2DObject。
 * 使用与城市标签相同的 projectPoint 投影，确保标记精确落在地图表面对应经纬度位置。
 * 标记自动跟随 OrbitControls 缩放/旋转/平移。
 */
function createDeviceMarkers(group: THREE.Group) {
  if (!props.devices.length) return;
  deviceMarkerElements.clear();
  props.devices.forEach((device) => {
    const anchor = projectPoint([device.lng, device.lat], 62);
    const wrapper = document.createElement('div');
    wrapper.className = 'device-marker-3d';
    ...
    deviceMarkerElements.set(device.id, wrapper);
...
function disposeDeviceMarkers() {
  deviceMarkerElements.forEach((el) => el.remove());
  deviceMarkerElements.clear();
  if (deviceMarkerGroup) {
    disposeObject3D(deviceMarkerGroup);
    deviceMarkerGroup = undefined;
  }
}
...
async function drillToFeature(featureName: string) {
  ...
  try {
    ...
    // 下钻后几何体重新投影居中，刷新缩放上下限以匹配新层级的自然视距，
    // 确保「缩到最远」阈值对本层级有效。
    applyZoomLimitsForCurrentScope();
  } catch (error) {
...
// 记录 pointerdown 的位置 / 时间，用于区分「点击下钻」与「拖拽平移」
let pointerDownX = 0;
let pointerDownY = 0;
...
function setup() {
  ...
  // 滚轮缩放：完全由外层 CockpitMap 接管（capture 拦截 + stopPropagation + 手动 zoomBy）。
  // 这里禁用 OrbitControls 内置滚轮缩放，彻底消除 damping 动画导致的 atMax 检测滞后。
  controls.enableZoom = false;
  controls.zoomSpeed = 0.9;
  controls.minDistance = 520;
  controls.maxDistance = 1450;
  // 左键拖拽 = 平移（查看城市）；右键拖拽 = 旋转（查看立体厚度）；中键 = 缩放
  ...
  if (!props.hideDrillControl) createDrillControl();
...
defineExpose({
  /** 返回上级地图层级 */
  drillBack: () => void drillBack(),
  /** 下钻到指定要素（省份/城市/区县） */
  drillTo: (name: string) => void drillToFeature(name),
  /** 当前悬停的地图要素名（用于滚轮钻取） */
  getHoveredFeature: () => hoveredFeature,
  /** 当前地图层级 */
  getScope: () => activeScope.value,
  /** 当前区域名称 */
  getRegionName: () => currentState.regionName,
  /** 是否可返回上级（有下钻栈） */
  canGoBack: () => drillStack.length > 0,
  /** 当前缩放状态（distance / min / max / atMin / atMax），供外层滚轮逐级返回判断 */
  getZoomState: () => getZoomState(),
  /** 手动缩放（外层 CockpitMap 拦截滚轮后调用），即时生效无 damping 延迟 */
  zoomBy: (deltaY: number) => zoomBy(deltaY),
});
...
.device-marker-3d .dm-dot {
  width: 10px;
  height: 10px;
  border-radius: 50%;
  background: var(--dm-color, #35D6C4);
  box-shadow: 0 0 10px var(--dm-color, #35D6C4), 0 0 2px rgba(0,0,0,0.6);
}
.device-marker-3d .dm-pulse {
  position: absolute;
  top: 0;
  left: 0;
  width: 10px;
  height: 10px;
  border-radius: 50%;
  border: 2px solid var(--dm-color, #35D6C4);
  animation: dm-pulse 2.2s ease-out infinite;
}
.device-marker-3d .dm-label {
  position: absolute;
  top: 14px;
  left: 50%;
  transform: translateX(-50%);
  ...
  border: 1px solid var(--dm-color, #35D6C4);
  border-radius: 4px;
  padding: 3px 7px;
...
.device-marker-3d.status-online { --dm-color: #35D6C4; }
.device-marker-3d.status-warning { --dm-color: #F0B94A; }
.device-marker-3d.status-alert { --dm-color: #FF6B5E; }
.device-marker-3d.status-offline { --dm-color: #5C6A7D; }
@keyframes dm-pulse {
