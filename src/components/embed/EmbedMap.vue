<!--
  ...
  Embed mode: 仅渲染 ChinaMap（无地球入场），供驾驶舱 iframe 嵌入。
-->
<template>
  <div class="embed-map">
    <ChinaMap :active="true" @ready="onReady" />
    <DeviceMarkers
      v-if="ready"
      :devices="devices"
...
import DeviceMarkers from './DeviceMarkers.vue';
...
/**
 * 默认设备示例数据。
 * 接入真实数据时，只需替换此数组或在父级通过 postMessage 注入。
 * 坐标范围：中国大陆约 73–135°E, 18–54°N
 */
