...
const earthViewRef = ref<{ flyToLocation?: (lon: number, lat: number, label?: string) => void } | null>(null);
...
defineExpose({
  ...
  /** AI 自然语言定位：在地球模式下飞行到指定经纬度并添加高亮标记 */
  flyToLocation: (lon: number, lat: number, label?: string) => {
    earthViewRef.value?.flyToLocation?.(lon, lat, label);
  },
  /** v2 数据层：地理坐标 → 屏幕坐标投影（只读，地球模式；不改视觉与相机） */
  projectGeoToScreen: (lon: number, lat: number) => {
    const view = earthViewRef.value as {
      projectGeoToScreen?: (lon: number, lat: number) => { x: number; y: number; visible: boolean };
    } | null;
    return view?.projectGeoToScreen?.(lon, lat) ?? { x: 0, y: 0, visible: false };
  },
});
</script>
