...
function createEarthTheme(primaryValue: THREE.ColorRepresentation) {
  ...
  return {
    ...
    // 真实地球大气层：自然蓝色，不跟随主题色
    atmosphere: new THREE.Color(0.25, 0.55, 1.0),
    outerAtmosphere: new THREE.Color(0.1, 0.3, 0.65),
    ...
    // 真实地球表面：自然海洋蓝 + 中性色阶，让卫星纹理自然呈现
    earthTint: new THREE.Color(0.08, 0.20, 0.42),
    earthGrade: new THREE.Color(0.92, 0.95, 1.0),
    oceanHighlight: new THREE.Color(0.35, 0.65, 0.95),
    earthCityLight: new THREE.Color(0.95, 0.80, 0.40),
    ...
    atmosphereSweep: new THREE.Color(0.3, 0.6, 1.0),
...
const rotationEpochMs = Date.UTC(2026, 0, 1);
...
const earthFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    vec3 gradedSurface = mix(daySurface, earthTint, 0.15) * uEarthGrade;
    float polarLatitude = smoothstep(0.67, 0.94, abs(vUv.y * 2.0 - 1.0));
    float polarIce = polarLatitude * smoothstep(0.3, 0.62, surfaceLuma);
    vec3 polarTone = vec3(surfaceLuma * 0.14, surfaceLuma * 0.18, surfaceLuma * 0.22);
    gradedSurface = mix(gradedSurface, polarTone, polarIce * 0.9);
    vec3 color = gradedSurface * (0.22 + wrappedLight * 0.62);
...
// ===== AI 自然语言定位：飞行 + 高亮标记 =====
let highlightMarker: THREE.Group | undefined;
let highlightMarkerStartTime = 0;
let flyToTimeline: gsap.core.Timeline | undefined;
let pendingFlyTo: { lon: number; lat: number; label?: string } | undefined;

function flyToLocation(lon: number, lat: number, label?: string) {
  console.log('[EarthView] flyToLocation called', lon, lat, label, 'introValue=', introValue, 'isTransitioning=', isTransitioning.value, 'transitionActive=', transitionTimeline?.isActive());
  if (!globeOrientation || !camera || !scene) {
    console.log('[EarthView] flyToLocation abort: no scene', { globeOrientation: !!globeOrientation, camera: !!camera, scene: !!scene });
    return;
  }
  // 入场动画未完成时排队，动画结束后自动执行
  if (introValue < 0.98) {
    console.log('[EarthView] flyToLocation queued, introValue=', introValue);
    pendingFlyTo = { lon, lat, label };
    return;
  }

  // 如果正在进入中国，不拦截
  if (isTransitioning.value && transitionTimeline?.isActive()) {
    console.log('[EarthView] flyToLocation abort: transition active');
    return;
  }

  console.log('[EarthView] flyToLocation proceeding');
  isTransitioning.value = true;
  if (controls) controls.enabled = false;
  hoverTarget = 0;

  // 计算目标四元数：使目标经纬度正对相机
  const displayDir = new THREE.Vector3(0, 0, 1);
  const regionDir = lonLatToVector3(lon, lat, 1).normalize();
  const alignRegion = new THREE.Quaternion().setFromUnitVectors(regionDir, displayDir);
  const rotatedNorth = new THREE.Vector3(0, 1, 0)
    .applyQuaternion(alignRegion)
    .projectOnPlane(displayDir)
    .normalize();
  const screenNorth = new THREE.Vector3(0, 1, 0)
    .projectOnPlane(displayDir)
    .normalize();
  const rollAngle = Math.atan2(
    displayDir.dot(rotatedNorth.clone().cross(screenNorth)),
    THREE.MathUtils.clamp(rotatedNorth.dot(screenNorth), -1, 1),
  );
  const removeRoll = new THREE.Quaternion().setFromAxisAngle(displayDir, rollAngle);
  const targetQuat = removeRoll.multiply(alignRegion);
  const startQuat = globeOrientation.quaternion.clone();

  // 相机拉近（缩小约 20%）
  const currentDist = camera.position.length();
  const targetDist = Math.max(currentDist * 0.80, 5.8);
  const camDir = camera.position.clone().normalize();
  const targetCamPos = camDir.multiplyScalar(targetDist);
  ...
  const proxy = { t: 0 };
  flyToTimeline = gsap.timeline({
    defaults: { ease: 'power2.inOut' },
    onComplete: () => {
      addHighlightMarker(lon, lat, label);
  ...
  flyToTimeline
    .to(proxy, {
      ...
      duration: 1.6,
      onUpdate: () => {
        globeOrientation?.quaternion.slerpQuaternions(startQuat, targetQuat, proxy.t);
      },
    }, 0)
    .to(camera.position, {
      x: targetCamPos.x,
...
function addHighlightMarker(lon: number, lat: number, _label?: string) {
  if (!globeOrientation || !scene) return;
  clearHighlightMarker();

  // 站点标记贴合地球表面（球面 2.0），紧贴但不陷入大气层（大气 2.045）
  // depthTest:false 保证标记不被大气层遮挡
  const MARKER_RADIUS = 2.03;
  const group = new THREE.Group();
  const pos = lonLatToVector3(lon, lat, MARKER_RADIUS);
  ...
  // 核心发光球 —— depthTest:false 确保始终可见，不被大气层遮挡
  ...
  const core = new THREE.Mesh(new THREE.SphereGeometry(0.035, 16, 12), coreMat);
  group.add(core);
  ...
  const ring = new THREE.Mesh(new THREE.RingGeometry(0.05, 0.072, 32), ringMat);
  ...
  const beam = new THREE.Mesh(
    new THREE.CylinderGeometry(0.006, 0.018, 0.32, 8, 1, true),
    beamMat,
  );
  ...
  // 提高 renderOrder 确保在大气辉光之后渲染
  group.traverse((child) => {
    if (child instanceof THREE.Mesh) child.renderOrder = 999;
  });

  globeOrientation.add(group);
  highlightMarker = group;
  highlightMarkerStartTime = animationElapsed;
  console.log('[EarthView] addHighlightMarker added at', lon, lat, 'radius', MARKER_RADIUS);
}

function clearHighlightMarker() {
  if (!highlightMarker) return;
  highlightMarker.traverse((child) => {
  ...
  highlightMarker.parent?.remove(highlightMarker);
  highlightMarker = undefined;
  highlightMarkerStartTime = 0;
}
...
function animate() {
  ...
  // 入场动画完成后，执行排队的 AI 定位请求
  if (pendingFlyTo && introValue >= 0.98 && !isTransitioning.value) {
    const p = pendingFlyTo;
    pendingFlyTo = undefined;
    console.log('[EarthView] executing pending flyTo', p);
    flyToLocation(p.lon, p.lat, p.label);
  }
  ...
  const earthRotationRemaining = 1 - THREE.MathUtils.smoothstep(introValue, 0.05, 0.45);
  ...
  if (idleDriftGroup) {
    ...
    idleDriftGroup.rotation.set(
      pitch * idleMotionValue
        ...
        + pointerParallaxPitch,
      yaw * idleMotionValue
        - THREE.MathUtils.degToRad(367) * earthRotationRemaining
        + pointerParallaxYaw,
  ...
  const rotationNow = (Date.now() - rotationEpochMs) / 1000;
  const lightPhase = rotationNow * fullTurn / 19;
  ...
  const chinaLightPhase = rotationNow * fullTurn / 31;
  ...
  // ===== AI 定位高亮标记动画 =====
  if (highlightMarker && highlightMarkerStartTime > 0) {
    const markerElapsed = animationElapsed - highlightMarkerStartTime;
    if (markerElapsed > 5) {
      clearHighlightMarker();
    } else {
      const fadeFactor = markerElapsed > 4 ? Math.max(0, 1 - (markerElapsed - 4)) : 1;
      const core = highlightMarker.children[0] as THREE.Mesh | undefined;
      const ring = highlightMarker.children[1] as THREE.Mesh | undefined;
      const beam = highlightMarker.children[2] as THREE.Mesh | undefined;
      if (core) {
        const corePulse = 1 + Math.sin(markerElapsed * 4) * 0.18;
        core.scale.setScalar(corePulse);
        const cm = core.material as THREE.MeshBasicMaterial;
        cm.opacity = 0.95 * fadeFactor;
      }
      if (ring) {
        const pulseProgress = (markerElapsed * 0.7) % 1;
        ring.scale.setScalar(1 + pulseProgress * 3.2);
        const rm = ring.material as THREE.MeshBasicMaterial;
        rm.opacity = Math.sin(pulseProgress * Math.PI) * 0.6 * fadeFactor;
      }
      if (beam) {
        const bm = beam.material as THREE.MeshBasicMaterial;
        bm.opacity = (0.28 + Math.sin(markerElapsed * 3) * 0.12) * fadeFactor;
      }
    }
  }
...
onBeforeUnmount(() => {
  ...
  chinaIntroPivot = undefined;
  flyToTimeline?.kill();
  flyToTimeline = undefined;
  pendingFlyTo = undefined;
  clearHighlightMarker();
});

const GLOBE_SURFACE_RADIUS = 2.0;

/**
 * 坐标投影（v2 数据层接入，§3.6.2 / §9.3-6）。
 * 将地理坐标经地球朝向旋转后投影为屏幕坐标，供地球模式资源聚合点覆盖层使用。
 * 只读访问相机与地球朝向，不改变任何视觉、材质、相机与动画状态。
 */
function projectGeoToScreen(lon: number, lat: number): { x: number; y: number; visible: boolean } {
  const hostEl = host.value;
  if (!hostEl || !camera || !renderer || !globeOrientation) {
    return { x: 0, y: 0, visible: false };
  }
  const localPos = lonLatToVector3(lon, lat, GLOBE_SURFACE_RADIUS);
  const worldPos = localPos.applyQuaternion(globeOrientation.quaternion).multiplyScalar(Math.max(globeOrientation.scale.x, 0.01));
  scene?.updateMatrixWorld();
  camera.updateMatrixWorld();
  const projected = worldPos.clone().project(camera);
  const width = Math.max(hostEl.clientWidth, 1);
  const height = Math.max(hostEl.clientHeight, 1);
  const x = (projected.x * 0.5 + 0.5) * width;
  const y = (-projected.y * 0.5 + 0.5) * height;
  const visible = projected.z < 1 && worldPos.z < (camera.position.z - 0.1);
  return { x, y, visible };
}

defineExpose({
  /** AI 自然语言定位：旋转地球使目标经纬度正对相机，并添加高亮标记 */
  flyToLocation,
  /** v2 数据层：地理坐标 → 屏幕坐标投影（只读） */
  projectGeoToScreen,
});
</script>
